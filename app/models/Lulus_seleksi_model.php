...
class Lulus_seleksi_model{
	...
	public function getLastYear(){
		$this->db->query("SELECT MAX(tahun) AS lastYear FROM ".$this->passed);
		$result = $this->db->single();
		return $result['lastYear'];
	}
	public function getYear(){
		$this->db->query("SELECT tahun FROM ".$this->passed." GROUP BY tahun ORDER BY tahun DESC");
		return $this->db->resultSet();
	}
	public function getMhsLast($tahun){
		$this->db->query("SELECT singkatan, IFNULL(total, 0) as total FROM ref_seleksi
						LEFT JOIN (
							SELECT kd_seleksi , COUNT(*) as total
							FROM ".$this->passed."
							WHERE tahun =:tahun
							GROUP BY kd_seleksi
							) AS ".$this->passed." ON ref_seleksi.kd_seleksi = ".$this->passed.".kd_seleksi
						GROUP BY singkatan
						ORDER BY ref_seleksi.kd_seleksi ASC");
	...
	public function getRowJenisKelamin($tahun, $jjar, $kolom, $fakultas){
		$this->db->query("SELECT $kolom AS kolom, 
						...
						LEFT JOIN (
							SELECT kd_jurusan, 
									SUM(CASE WHEN kd_seleksi='S01' AND jenis_kelamin='L' THEN 1 ELSE 0 END) AS jumlah_laki_lakiS01,
									SUM(CASE WHEN kd_seleksi='S01' AND jenis_kelamin='P' THEN 1 ELSE 0 END) AS jumlah_perempuanS01,

									SUM(CASE WHEN kd_seleksi='S02' AND jenis_kelamin='L' THEN 1 ELSE 0 END) AS jumlah_laki_lakiS02,
									SUM(CASE WHEN kd_seleksi='S02' AND jenis_kelamin='P' THEN 1 ELSE 0 END) AS jumlah_perempuanS02,

									SUM(CASE WHEN kd_seleksi='S03' AND jenis_kelamin='L' THEN 1 ELSE 0 END) AS jumlah_laki_lakiS03,
									SUM(CASE WHEN kd_seleksi='S03' AND jenis_kelamin='P' THEN 1 ELSE 0 END) AS jumlah_perempuanS03
							FROM ".$this->passed."
							WHERE tahun =:tahun
							GROUP BY kd_jurusan
							) AS ".$this->passed." ON ".$this->jurusan.".kd_jurusan = ".$this->passed.".kd_jurusan
	...
	public function getRowJenisKelamin5y($jjar, $kolom, $fakultas, $seleksi){
		$this->db->query("SELECT $kolom AS kolom, 
						...
						LEFT JOIN (
							SELECT kd_jurusan, 
									SUM(CASE WHEN tahun=:tahun5 AND jenis_kelamin = 'L' THEN 1 ELSE 0 END) AS total5_L,
									SUM(CASE WHEN tahun=:tahun5 AND jenis_kelamin = 'P' THEN 1 ELSE 0 END) AS total5_P,
									SUM(CASE WHEN tahun=:tahun4 AND jenis_kelamin = 'L' THEN 1 ELSE 0 END) AS total4_L,
									SUM(CASE WHEN tahun=:tahun4 AND jenis_kelamin = 'P' THEN 1 ELSE 0 END) AS total4_P,
									SUM(CASE WHEN tahun=:tahun3 AND jenis_kelamin = 'L' THEN 1 ELSE 0 END) AS total3_L,
									SUM(CASE WHEN tahun=:tahun3 AND jenis_kelamin = 'P' THEN 1 ELSE 0 END) AS total3_P,
									SUM(CASE WHEN tahun=:tahun2 AND jenis_kelamin = 'L' THEN 1 ELSE 0 END) AS total2_L,
									SUM(CASE WHEN tahun=:tahun2 AND jenis_kelamin = 'P' THEN 1 ELSE 0 END) AS total2_P,
									SUM(CASE WHEN tahun=:tahun1 AND jenis_kelamin = 'L' THEN 1 ELSE 0 END) AS total1_L,
									SUM(CASE WHEN tahun=:tahun1 AND jenis_kelamin = 'P' THEN 1 ELSE 0 END) AS total1_P
							FROM ".$this->passed."
							LEFT JOIN ".$this->seleksi." ON ".$this->passed.".kd_seleksi = ".$this->seleksi.".kd_seleksi
	...
	public function getRowDaerahAsal($tahun, $yesLike, $notLike, $null){
		$this->db->query("SELECT ".$this->provinsi.".kd_provinsi, ".$this->provinsi.".nama_provinsi, 
						...
						LEFT JOIN (
							...
							WHERE tahun=:tahun
							GROUP BY kd_kota
						) AS ".$this->passed." ON ".$this->kota.".kd_kota = ".$this->passed.".kd_kota
	...
	public function getTotal($jjar, $kolom, $fakultas){
		$this->db->query("SELECT $kolom AS kolom , ".$this->fakultas.".fakultas,
						...
						LEFT JOIN (
							SELECT kd_jurusan, 
									SUM(CASE WHEN tahun=:tahun7 THEN 1 ELSE 0 END) AS tahun7,
									SUM(CASE WHEN tahun=:tahun6 THEN 1 ELSE 0 END) AS tahun6,
									SUM(CASE WHEN tahun=:tahun5 THEN 1 ELSE 0 END) AS tahun5,
									SUM(CASE WHEN tahun=:tahun4 THEN 1 ELSE 0 END) AS tahun4,
									SUM(CASE WHEN tahun=:tahun3 THEN 1 ELSE 0 END) AS tahun3,
									SUM(CASE WHEN tahun=:tahun2 THEN 1 ELSE 0 END) AS tahun2,
									SUM(CASE WHEN tahun=:tahun1 THEN 1 ELSE 0 END) AS tahun1
							FROM ".$this->passed."
							GROUP BY kd_jurusan
						) AS ".$this->passed." ON ".$this->jurusan.".kd_jurusan = ".$this->passed.".kd_jurusan
						LEFT JOIN ".$this->fakultas." ON ".$this->jurusan.".fakultas = ".$this->fakultas.".fakultas
